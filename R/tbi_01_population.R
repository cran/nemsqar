...
tbi_01_population <- function(
  ...
  if (
    ...
    cli::cli_abort(
      "{.fn tbi_01_population} requires either a {.cls data.frame} or {.cls tibble} passed to the {.var df} argument, or all table arguments to be fulfilled. Please choose one approach."
    )
  }
  ...
  if (
    ...
    cli::cli_abort(
      "One or more of the *_col arguments is missing. Please ensure you pass an unquoted column to each of the *_col arguments to run {.fn tbi_01_population}."
    )
  }
  ...
  computing_population <- final_data |>
    dplyr::mutate(
      ...
      PROVIDER_IMPRESSION = {{ erecord_01_col }} %in%
        provider_impression_data,
  ...
  filter_counts <- tibble::tibble(
    ...
    count = c(
      ...
      sum(computing_population$PROVIDER_IMPRESSION, na.rm = TRUE),
