...
#' @export
#'
respiratory_02_population <- function(
  df = NULL,
  ...
  if (
    ...
    cli::cli_abort(
      "{.fn respiratory_02_population} will only work by passing a {.cls data.frame} or {.cls tibble} to the {.var df} argument, or by fulfilling all table arguments.  Please choose to either pass an object of class {.cls data.frame} or {.cls tibble} to the {.var df} argument, or fulfill all table arguments."
    )
  }
  ...
  if (
    ...
    cli::cli_abort(
      "One or more of the *_col arguments is missing.  Please make sure you pass an unquoted column to each of the *_col arguments to run {.fn respiratory_02_population}."
    )
  }
  ...
  if (
    ...
    final_data <- patient_scene_table |>
      ...
      dplyr::mutate(
        ...
        system_age_minor_exclusion1 = {{ epatient_15_col }} < 24 &
          grepl(
            pattern = hour_values,
            x = {{ epatient_16_col }},
            ignore.case = TRUE
          ),
        system_age_minor_exclusion2 = {{ epatient_15_col }} < 120 &
          grepl(
            pattern = minute_values,
            x = {{ epatient_16_col }},
            ignore.case = TRUE
          ),
        system_age_minor_exclusion = system_age_minor_exclusion1 |
          system_age_minor_exclusion2,
