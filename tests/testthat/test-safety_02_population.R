...
testthat::test_that("safety_02_population rejects missing required column arguments", {
  testthat::expect_error(
    safety_02_population(df = tibble::tibble(), epatient_15_col = "Age"),
    "One or more of the \\*_col arguments is missing"
  )
})

testthat::test_that("safety_02_population rejects non-dataframe inputs", {
  testthat::expect_error(safety_02_population(df = list()), "One or more")

  testthat::expect_error(
    safety_02_population(patient_scene_table = matrix()),
    "One or more"
  )
})
