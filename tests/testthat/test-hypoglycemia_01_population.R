...
testthat::test_that("hypoglycemia_01_population rejects missing required column arguments", {
  testthat::expect_error(
    hypoglycemia_01_population(df = tibble::tibble(), epatient_15_col = "Age"),
    "One or more of the \\*_col arguments is missing"
  )
})

testthat::test_that("hypoglycemia_01_population rejects non-dataframe inputs", {
  testthat::expect_error(hypoglycemia_01_population(df = list()), "One or more")

  testthat::expect_error(
    hypoglycemia_01_population(patient_scene_table = matrix()),
    "One or more"
  )
})

testthat::test_that("hypoglycemia_01_population validates date column formats", {
  # Fake df for test 1 (with real columns)
  df <- tibble::tibble(
    ...
    evitals_18 = rnorm(5),
    evitals_23 = rnorm(5),
    evitals_26 = rep("val", 5),
    eprocedures_03 = rep("p1", 5)
  )

  # Expect an error due to incorrect date classes
  testthat::expect_error(
    hypoglycemia_01_population(
      df,
  ...
  # Fake empty tables for test 2 (correct for your NULL branch)
  patient_scene_table <- tibble::tibble(
  ...
  # Expect NULL because date columns do not actually exist in tables
  testthat::expect_error(
    hypoglycemia_01_population(
...
testthat::test_that("hypoglycemia_01_population correctly classifies patient age", {
  ...
  result <- hypoglycemia_01_population(
    df,
    erecord_01_col = erecord_01,
    incident_date_col = incident_date,
    patient_DOB_col = patient_dob,
