...
testthat::test_that("Test for df argument with wrong format", {
  ...
  testthat::expect_error(
    ...
    regexp = "must be of class.*data\\.frame.*tbl.*tbl_df"
  )
})
...
testthat::test_that("Test for incorrectly formatted DOB / incident date", {
  ...
  wrong_data$`Incident Date` <- as.character(
    nemsqar_patient_scene_table$`Incident Date`
  )
  wrong_data$`Patient Date Of Birth (ePatient.17)` <- as.character(
    nemsqar_patient_scene_table$`Patient Date Of Birth (ePatient.17)`
  )
...
testthat::test_that("Test for correctly formatted dates/times in df", {
  ...
  big_df <- tibble::tibble(
    `Incident Patient Care Report Number - PCR (eRecord.01)` = c(
      "a",
      "b",
      "c",
      "d",
    ...
    `Patient Age Units (ePatient.16)` = c(
      "Years",
      "Years",
      "Years",
      "Years",
    ...
    `Response Type Of Service Requested With Code (eResponse.05)` = rep(
      "2205001",
      5
    ),
    ...
    `Procedure Performed Description And Code (eProcedures.03)` = rep(
      "112798008",
      5
    ),
    ...
    `Airway Device Placement Confirmed Method List (eAirway.04)` = rep(
      "4004019",
      5
    ),
...
testthat::test_that("Test for incorrectly formatted vitals/airway times", {
  ...
  wrong_data_vitals$`Vitals Signs Taken Date Time (eVitals.01)` <- as.character(
    nemsqar_vitals_table$`Vitals Signs Taken Date Time (eVitals.01)`
  )
  wrong_data_proc$`Procedure Performed Date Time (eProcedures.01)` <- as.character(
    nemsqar_procedures_table$`Procedure Performed Date Time (eProcedures.01)`
  )
  wrong_data_airway$`Airway Device Placement Confirmation Date Time (eAirway.02)` <- as.character(
    nemsqar_airway_table$`Airway Device Placement Confirmation Date Time (eAirway.02)`
  )
